...
const filenameValidator = (filename) => {
  const handler = [() => filename.startsWith('.')].filter((result) => result() === false)
  return handler.length > 0
}

const checkFiles = (filenames) =>
  filenames
    .map((f) => {
      const filename = path.relative(process.cwd(), f)
      if (filenameValidator(filename)) return filename
    })
    .filter(function (x) {
      return x !== undefined
    })
...
const buildEslintCommand = (filenames) => {
  ...
  if (files?.length > 0) {
    return `yarn lint:staged ${files.map((f) => path.relative(process.cwd(), f)).join(' ')}`
  }
...
const buildTestCommand = (filenames) => {
  const files = checkFiles(filenames)
  if (files?.length > 0) {
    return `yarn test:staged ${files.map((f) => path.relative(process.cwd(), f)).join(' ')}`
  }
